...
pub fn init_project(target_dir: &Path) -> Result<(), String> {
    ...
    write_if_missing_yaml(&snapshot_dir.join(FILE_SNAPSHOT_MANIFEST), &snapshot_manifest)?;
    write_if_missing_yaml(&snapshot_dir.join(FILE_SNAPSHOT_ENTITIES), &entities_manifest)?;
    write_if_missing_yaml(&snapshot_dir.join(FILE_SNAPSHOT_MESSAGES), &messages_manifest)?;
...
mod tests {
    ...
    fn init_project_creates_required_loader_files() {
        ...
        assert!(target.join(DIR_SNAPSHOTS).join(INITIAL_SNAPSHOT_ID).join(FILE_SNAPSHOT_MANIFEST).exists());
        assert!(target.join(DIR_SNAPSHOTS).join(INITIAL_SNAPSHOT_ID).join(FILE_SNAPSHOT_ENTITIES).exists());
        assert!(target.join(DIR_SNAPSHOTS).join(INITIAL_SNAPSHOT_ID).join(FILE_SNAPSHOT_MESSAGES).exists());
