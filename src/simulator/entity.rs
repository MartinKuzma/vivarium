...
impl Entity {
    ...
    pub fn update(&mut self) -> Result<(), String> {
        let msgs_table = self.lua_script_manager.lua_vm
        .create_table()
        .map_err(|e| {e.to_string()})?;

        while let Some(msg) = self.lua_script_manager.messages.pop() {
        ...
        let result = self.lua_script_manager
            .lua_vm
    ...
    pub fn receive_message(&mut self, message: crate::simulator::messaging::Message) {
        self.lua_script_manager.messages.push(message.clone());
    }
}
